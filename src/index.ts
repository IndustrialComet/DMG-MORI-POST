...
//M1 Optional Stop   //Allows the operator to stop and play when they were ready
//
...
/**
	Output a comment.
*/
function writeComment(text: Value) {
...
function onOpen() {
	...
	if (properties.forceIJK) {
		outputs.i = createReferenceVariable({prefix:"I",force:true},formats.xyz);
		outputs.j = createReferenceVariable({prefix:"J",force:true},formats.xyz);
		outputs.k = createReferenceVariable({prefix:"K",force:true},formats.xyz);
	}
...
function onComment(message: Value) {
	var comments = String(message).split(";").forEach(comment=> {
	writeComment(comment);
	});
}
...
/*
	...
	*/
		...
		writeBlock("T" + formats.tool.format(tool.number),formats.m.format(6),tool.desciption === undefined?"":tool.desciption)//conditional(tool.description,formatComment(tool.description)));
		if (tool.comment) {
	...
	*/
	...
	if (!isProbeOperation() &&
			(insertToolCall ||
			 forceSpindleSpeed ||
			 isFirstSection() ||
			 (formats.rpm.areDifferent(tool.spindleRPM,outputs.spindle.getCurrent() as number)) ||
		...
		//writeBlock(
		 // outputs.spindle.format(tool.spindleRPM),formats.m.format(tool.isClockwise() ? 3 : 4)
		//);
	...
	if (insertToolCall || !lengthCompensationActive || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
		...
		if(forceSpindleSpeed ||
			...
			(tool.isClockwise() != getPreviousSection().getTool().clockwise)) {
		 writeBlock(
			modals.abs.format(90),
			modals.motion.format(0),
			outputs.x.format(initialPosition.x),
			outputs.y.format(initialPosition.y),outputs.spindle.format(tool.spindleRPM),formats.m.format(tool.isClockwise() ? 3 : 4),setCoolant(tool.coolant)
		 );
		} else {
			writeBlock(setCoolant(tool.coolant));
			writeBlock(
...
//1
function onSpindleSpeed(spindleSpeed: number) {
	writeBlock(outputs.spindle.format(spindleSpeed));
}

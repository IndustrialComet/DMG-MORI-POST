...
setCodePage("ascii");
capabilities = CONSTANT.CAPABILITY.MILLING;
tolerance = spatial(0.002, CONSTANT.UNIT.MILLIMETER);
minimumChordLength = spatial(0.01, CONSTANT.UNIT.MILLIMETER);
minimumCircularRadius = spatial(0.01, CONSTANT.UNIT.MILLIMETER);
maximumCircularRadius = spatial(1000, CONSTANT.UNIT.MILLIMETER);
...
var formats = {
    ...
    //DRILLING
    feed: createFormat({ decimals: (unit == CONSTANT.UNIT.MILLIMETER ? 0 : 1), forceDecimal: true }),
    pitch: createFormat({ decimals: (unit == CONSTANT.UNIT.MILLIMETER ? 3 : 4), forceDecimal: true }),
            ...
            for (var j = i + 1; j < getNumberOfSections(); ++j) {
                ...
                if (tooli.number == toolj.number) {
                    if (formats.xyz.areDifferent(tooli.diameter, toolj.diameter) ||
                        ...
                        error(subst(localize("Using the same tool number for different cutter geometry for operation '%1' and '%2'."), sectioni.hasParameter("operation-comment") ? sectioni.getParameter("operation-comment") : ("#" + (i + 1)), sectionj.hasParameter("operation-comment") ? sectionj.getParameter("operation-comment") : ("#" + (j + 1))));
                        return;
                    }
                }
            }
...
class FeedContext {
    constructor(id, description, feed) {
        this.id = id;
...
function setWorkPlane(abc) {
    ...
    onCommand(CONSTANT.COMMAND.UNLOCK_MULTI_AXIS);
    ...
    onCommand(CONSTANT.COMMAND.LOCK_MULTI_AXIS);
...
function onSection() {
    ...
    if (insertToolCall || newWorkOffset || newWorkPlane || forceSmoothing) {
        ...
        */
        ...
        retracted = true;
        onCommand(CONSTANT.COMMAND.COOLANT_OFF);
    ...
    if (insertToolCall) {
        ...
        if (properties.optionalStop) {
            onCommand(CONSTANT.COMMAND.OPTIONAL_STOP);
        }
        /*
    ...
    if (insertToolCall || !lengthCompensationActive || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
        ...
        if (!machineConfiguration.isHeadConfiguration()) {
            writeBlock(modals.abs.format(0), modals.abs.format(90), wfo, outputs.x.format(initialPosition.x), outputs.y.format(initialPosition.y), outputs.s.format(tool.spindleRPM), formats.m.format(tool.clockwise ? 3 : 4));
            writeBlock(modals.motion.format(0), conditional(insertToolCall, formats.g.format(currentSection.isMultiAxis() ? 43.5 : 43)), formats.h.format(lengthOffset), outputs.z.format(initialPosition.z), setCoolant(tool.coolant), conditional(insertToolCall), conditional(nextTool, "T" + nextTool));
            //writeBlock(setCoolant(tool.coolant));
            lengthCompensationActive = true;
        }
        else {
            writeBlock(modals.abs.format(90), modals.motion.format(0), formats.g.format(currentSection.isMultiAxis() ? (machineConfiguration.isMultiAxisConfiguration() ? 43.4 : 43.5) : 43), formats.h.format(lengthOffset), outputs.x.format(initialPosition.x), outputs.y.format(initialPosition.y), outputs.z.format(initialPosition.z), setCoolant(tool.coolant), conditional(insertToolCall), conditional(nextTool, "T" + nextTool));
        }
        modals.motion.reset();
    }
    else {
        if (forceSpindleSpeed ||
            (formats.rpm.areDifferent(tool.spindleRPM, outputs.s.getCurrent())) ||
            (tool.clockwise != getPreviousSection().getTool().clockwise)) {
            writeBlock(modals.abs.format(90), modals.motion.format(0), outputs.x.format(initialPosition.x), outputs.y.format(initialPosition.y), outputs.s.format(tool.spindleRPM), formats.m.format(tool.clockwise ? 3 : 4), setCoolant(tool.coolant));
        }
        else {
            writeBlock(setCoolant(tool.coolant));
...
function onCyclePoint(x, y, z) {
    ...
    if (isFirstCyclePoint()) {
        ...
        switch (cycleType) {
            case "drilling":
                writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(81), getCommonCycle(x, y, z, cycle.retract), outputs.feed.format(F));
                break;
            case "counter-boring":
                if (P > 0) {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(82), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), outputs.feed.format(F));
                }
                else {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(81), getCommonCycle(x, y, z, cycle.retract), outputs.feed.format(F));
                }
            ...
            case "chip-breaking":
                ...
                if (P > 0) {
                    ...
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(73), getCommonCycle(x, y, z, cycle.retract), "Q" + formats.xyz.format(cycle.incrementalDepth), outputs.feed.format(F));
                }
                break;
            case "deep-drilling":
                if (P > 0) {
                    ...
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(83), getCommonCycle(x, y, z, cycle.retract), "Q" + formats.xyz.format(cycle.incrementalDepth), 
            ...
            case "tapping":
                ...
                if (properties.usePitchForTapping) {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.feed.format(95), modals.cycle.format((tool.type == TOOL_TAP_LEFT_HAND) ? 74 : 84), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), pitcoutputs.h.format(tool.threadPitch));
                    forceFeed();
                }
                else {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format((tool.type == TOOL_TAP_LEFT_HAND) ? 74 : 84), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), outputs.feed.format(F));
                }
                break;
            case "left-tapping":
                writeBlock(formats.m.format(29), outputs.s.format(tool.spindleRPM));
                if (properties.usePitchForTapping) {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.feed.format(95), modals.cycle.format(74), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), pitcoutputs.h.format(tool.threadPitch));
                    forceFeed();
                }
                else {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(74), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), outputs.feed.format(properties.useG95 ? tool.getTappingFeedrate() / spindleSpeed : tool.getTappingFeedrate()));
                }
                break;
            case "right-tapping":
                writeBlock(formats.m.format(29), outputs.s.format(tool.spindleRPM));
                if (properties.usePitchForTapping) {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(84), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), pitcoutputs.h.format(tool.threadPitch));
                    forceFeed();
                }
                else {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(84), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), outputs.feed.format(properties.useG95 ? tool.getTappingFeedrate() / spindleSpeed : tool.getTappingFeedrate()));
                }
                break;
            ...
            case "right-tapping-with-chip-breaking":
                writeBlock(formats.m.format(29), outputs.s.format(tool.spindleRPM));
                if (properties.usePitchForTapping) {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format((tool.type == TOOL_TAP_LEFT_HAND ? 74 : 84)), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), "Q" + formats.xyz.format(cycle.incrementalDepth), pitcoutputs.h.format(tool.threadPitch));
                    forceFeed();
                }
                else {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format((tool.type == TOOL_TAP_LEFT_HAND ? 74 : 84)), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), "Q" + formats.xyz.format(cycle.incrementalDepth), outputs.feed.format(properties.useG95 ? tool.getTappingFeedrate() / spindleSpeed : tool.getTappingFeedrate()));
                }
                break;
            case "fine-boring":
                writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(76), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), // not optional
                "Q" + formats.xyz.format(cycle.shift), outputs.feed.format(F));
                break;
            case "back-boring":
                ...
                writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(87), getCommonCycle(x - dx, y - dy, z - dz, cycle.bottom), "Q" + formats.xyz.format(cycle.shift), "P" + formats.milli.format(P), // not optional
                outputs.feed.format(F));
                break;
            case "reaming":
                if (P > 0) {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(89), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), outputs.feed.format(F));
                }
                else {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(85), getCommonCycle(x, y, z, cycle.retract), outputs.feed.format(F));
                }
                break;
            case "stop-boring":
                if (P > 0) {
                    ...
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(86), getCommonCycle(x, y, z, cycle.retract), outputs.feed.format(F));
                }
                break;
            case "manual-boring":
                writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(88), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), // not optional
                outputs.feed.format(F));
                break;
            case "boring":
                if (P > 0) {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(89), getCommonCycle(x, y, z, cycle.retract), "P" + formats.milli.format(P), // not optional
                    outputs.feed.format(F));
                }
                else {
                    writeBlock(modals.retraction.format(98), modals.abs.format(90), modals.cycle.format(85), getCommonCycle(x, y, z, cycle.retract), outputs.feed.format(F));
...
var currentCoolantMode = CONSTANT.COOLANT.DISABLED;
function setCoolant(coolant) {
    if (isProbeOperation()) { // avoid coolant output for probing
        coolant = CONSTANT.COOLANT.DISABLED;
    }
    if (coolant == currentCoolantMode) {
        return; // coolant is already active
    }
    if (coolant == CONSTANT.COOLANT.DISABLED) {
        writeBlock(formats.m.format((currentCoolantMode == CONSTANT.COOLANT.THROUGH_TOOL) ? 89 : 9));
        currentCoolantMode = CONSTANT.COOLANT.DISABLED;
        return;
    }
    var m;
    switch (coolant) {
        case CONSTANT.COOLANT.FLOOD:
            m = 8;
            break;
        case CONSTANT.COOLANT.THROUGH_TOOL:
            m = 88;
            break;
        default:
...
function onCommand(command) {
    switch (command) {
        case 1:
            setCoolant(CONSTANT.COOLANT.DISABLED);
            return;
        case RECORD_CIRCULAR: //////////////REMOVE /  /.............
            return;
        case CONSTANT.COMMAND.COOLANT_ON:
            setCoolant(COOLANT_FLOOD);
            return;
        case CONSTANT.COMMAND.STOP:
            writeBlock(formats.m.format(0));
            forceSpindleSpeed = true;
            return;
        case CONSTANT.COMMAND.START_SPINDLE:
            onCommand(tool.clockwise ? CONSTANT.COMMAND.SPINDLE_CLOCKWISE : CONSTANT.COMMAND.SPINDLE_COUNTERCLOCKWISE);
            return;
        case CONSTANT.COMMAND.LOCK_MULTI_AXIS:
            return;
        case CONSTANT.COMMAND.UNLOCK_MULTI_AXIS:
            return;
        case CONSTANT.COMMAND.START_CHIP_TRANSPORT:
            return;
        case CONSTANT.COMMAND.STOP_CHIP_TRANSPORT:
            return;
        case CONSTANT.COMMAND.BREAK_CONTROL:
            ...
            return;
        case CONSTANT.COMMAND.TOOL_MEASURE:
            return;
    }
...
function onSectionEnd() {
    if ((((getCurrentSectionId() + 1) >= getNumberOfSections()) ||
        ...
        tool.breakControl) {
        onCommand(CONSTANT.COMMAND.BREAK_CONTROL);
    }
    else {
        toolChecked = false;
    }
...
function onClose() {
    ...
    onImpliedCommand(CONSTANT.COMMAND.END);
    onImpliedCommand(CONSTANT.COMMAND.STOP_SPINDLE);

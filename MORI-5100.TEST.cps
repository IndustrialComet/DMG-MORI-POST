...
writeComment((now.getMonth()+1) + "/" + now.getDate() + "/" + now.getFullYear() + " " + now.getHours() + ":" + ('0'+now.getMinutes()).slice(-2));
  ...
  writeBlock(gPlaneModal.format(17), gFormat.format(40), gFormat.format(80), gAbsIncModal.format(90));
...
function setSmoothing(mode) {
 ...
 // validate(!lengthCompensationActive, "Length compensation is active while trying to update smoothing.");
  ...
  writeBlock(gFormat.format(332), "R3.");
  return true;
}
...
function onSection() {
  ...
  var forceSmoothing =  properties.useSmoothing &&
	...
	writeComment(insertToolCall.toString());
	writeComment(newWorkOffset.toString());
	writeComment(newWorkPlane.toString());
	writeComment(forceSmoothing.toString());
   ...
   writeComment("b1");
    ...
    writeBlock(gFeedModeModal.format(0), gFormat.format(49)); 
    writeBlock(gFormat.format(53), "Z" + xyzFormat.format(0), mFormat.format(19)); // retract
...
/*
  ...
  if (properties.preloadTool) {
      var nextTool = getNextTool(tool.number);
      if (nextTool) {
        nextTool = toolFormat.format(nextTool.number); //Store next tool
      } else {
        // preload first tool
        var section = getSection(0);
        var firstToolNumber = section.getTool().number;
        if (tool.number != firstToolNumber) {
          nextTool = toolFormat.format(firstToolNumber);//Store next tool
        }
      }
    }
    ...
    if (workOffset > 6) {
      ...
      if (p > 300) {
        ...
        return;
          ...
          var wfo =(gFormat.format(54.1) + " P" + p); // G54.1P
  ...
  if (insertToolCall || !lengthCompensationActive || retracted || (!isFirstSection() && getPreviousSection().isMultiAxis())) {
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
		writeComment("d3");
      writeBlock(
        gAbsIncModal.format(0),
        gAbsIncModal.format(90), wfo, xOutput.format(initialPosition.x), yOutput.format(initialPosition.y),
		sOutput.format(tool.spindleRPM), mFormat.format(tool.clockwise ? 3 : 4));
     writeBlock(
        gMotionModal.format(0), 
        conditional(insertToolCall, gFormat.format(currentSection.isMultiAxis() ? 43.5 : 43)), 
    ...
    if(forceSpindleSpeed ||
     ...
     writeBlock(
       ...
       gMotionModal.format(0),
       xOutput.format(initialPosition.x),
       yOutput.format(initialPosition.y), sOutput.format(tool.spindleRPM), mFormat.format(tool.clockwise ? 3 : 4), setCoolant(tool.coolant)
     );
    } else {
      writeBlock(setCoolant(tool.coolant));
      writeBlock(
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(gAbsIncModal.format(90), gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3),
        iOutput.format(circle.offset.x, 0), jOutput.format(circle.offset.y, 0),
        getFeed(feed)
      );
      break;
    case PLANE_ZX:
      writeBlock(gAbsIncModal.format(90), gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3),
        iOutput.format(circle.offset.x, 0), kOutput.format(circle.offset.z, 0),
        getFeed(feed)
      );
      break;
    case PLANE_YZ:
      writeBlock(gAbsIncModal.format(90), gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3),
        jOutput.format(circle.offset.y, 0), kOutput.format(circle.offset.z, 0),
        getFeed(feed)
      );
      break;
    default:
...
function setCoolant(coolant) {
  ...
  if (m) {
    //writeBlock(mFormat.format(m)); //take out the writeblock contained in the function
    currentCoolantMode = coolant;
	return mFormat.format(m); //output as mFormat instead of block 
  }
}
...
function onCommand(command) {
  ...
  case COMMAND_BREAK_CONTROL:
    if (!toolChecked) { // avoid duplicate COMMAND_BREAK_CONTROL
      ...
      onCommand(COMMAND_COOLANT_OFF);
      writeBlock(gFormat.format(53), "Z" + xyzFormat.format(0), mFormat.format(19)); // retract
      writeBlock(gFormat.format(8), pFormat.format(0)); 
        writeBlock(
        gFormat.format(65),
        "P" + 9858,
...
function onClose() {
  ...
  if (!machineConfiguration.hasHomePositionX() && !machineConfiguration.hasHomePositionY()) {
    ...
    writeBlock(gFormat.format(53), "X" + xyzFormat.format(-25.)); // return to home
    writeBlock(gFormat.format(53), "Y" + xyzFormat.format(0)); // return to home
  } else {
    var homeX;
